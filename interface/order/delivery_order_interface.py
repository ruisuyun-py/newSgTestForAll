...
def get_delivery_order_info(query_params_dict, return_info_list):
    ...
    params = {
        'ModelTypeName': 'ErpWeb.Domain.ViewModels.Deliverys.DeliveryVmv',
        "Input2Name": "Tid",
        "listType": "1",
        "": "",
    }
    for k, v in query_params_dict.items():
        params[info_map[k]] = v
    url = "http://gw.erp12345.com/api/Deliverys/Delivery/QueryPage?"
    for k, v in params.items():
        url += f"{k}={v}&"
    headers = {
        'Cookie': base.cookies
...
# 系统出库
def send_delivery(delivery_order_id):
    """
     delivery_order_id: 发货单ID
     """
    url = "http://gw.erp12345.com/api/Deliverys/Delivery/SysSendDelivery?"
    params = {
        "deliveryIds": delivery_order_id
    }
    for k, v in params.items():
        url += f"{k}={v}"
    headers = {
